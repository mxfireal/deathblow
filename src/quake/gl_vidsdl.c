...
static qboolean VID_SetMode(int width, int height, int refreshrate, int bpp, qboolean fullscreen)
{
        ...
        // sorry for littering but if this stuff doesnt work for you for whatever reason you can easily go back
...
// respectfully i dont give a darn
#if 0
	if (!VID_ValidMode (width, height, refreshrate, bpp, fullscreen))
	{
        ...
        vid.conwidth = (scr_conwidth.value > 0) ? (int)scr_conwidth.value : (scr_conscale.value > 0) ? (int)(vid.width / scr_conscale.value)
                                                                                                     : vid.width;
...
#define num_readvars Q_COUNTOF(read_vars)
        ...
        if (CFG_OpenConfig("./local/config.cfg") == 0)
        {
