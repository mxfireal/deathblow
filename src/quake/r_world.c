...
//    making Vert a vec4. (https://sourceforge.net/p/quakespasm/bugs/39/)
	...
	const GLchar *fragSource = \
		...
		"}\n";
#else
        const GLchar *fragSource = PrRs_Lookup("qspFS");
        const GLchar *vertSource = PrRs_Lookup("qspVS");
#endif

        if (!gl_glsl_alias_able)
                return;

        r_world_program = GL_CreateProgram(vertSource, fragSource, /*Q_COUNTOF(bindings)*/ 0, bindings);
        ...
        if (r_world_program != 0)
        {
                ...
                if (texLoc == -1 && LMTexLoc == -1)
                {
                        Con_Warning("failed to get gl uniforms");
                        return;
                }
        }
        else
