...
void Cmd_Exec_f (void)
{
	...
	if (Cmd_Argc () != 2)
	{
        ...
        //if (!f)
        {
                ...
                static const char* exec_search[] = {
                        "./local/cfg/%s",
                        "./local/%s",
                        "cfg/%s", //relative to content
                        "%s", //^
                        "./cfg/%s", //how valve does it
                        "./id1/%s", //for stock quake config.cfg
                        "./qw/%s", //for quakeworld's config.cfg
                        "./%s", //scan the root directory of the game as a last resort
                        0
                };
                ...
                int handle;
                int r;
                while (exec_search[counter])
                {
                        const char* ref=exec_search[counter];
                        const char* format_prefix = ".cfg";
                        char formatstr[128];
                        snprintf(formatstr,128,"%s%s",ref,format_prefix);
                        snprintf(minibuf,128,formatstr,Cmd_Argv(1));
                        r = COM_FileExists(minibuf,0);
                        if (r) 
                        {
                                goto label_skipsecond;
                        }
                        counter++;
                        
                }
                #if 1 //needed for quake.rc, LAME!
                while (exec_search[counter])
                {
                        const char* ref=exec_search[counter];
                        const char* format_prefix = ""; //Nothing
                        char formatstr[128];
                        snprintf(formatstr,128,"%s%s",ref,format_prefix);
                        snprintf(minibuf,128,formatstr,Cmd_Argv(1));
                        r = COM_FileExists(minibuf,0);
                ...
                {
                        r = COM_FOpenFile(minibuf,&file,0);
                        if (r == -1)
                        {
                                //whatever
                                file = fopen(minibuf,"rb");
                        }

                        f = QSIO_allocfile(&file,0);
	...
	if (!f)
	{
        ...
        Con_Printf("warning: loading config file %s using quake",Cmd_Argv(1));
	Con_Printf ("execing %s\n",Cmd_Argv(1));
